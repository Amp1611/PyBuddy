...
def start_file9():
    class RiddleQuizGUI(wx.Frame):
        def __init__(self):
            super().__init__(None, title="Python Riddle Quiz", size=(700, 400))
            self.Centre()
            ...
            self.question_label = wx.StaticText(panel, label="", style=wx.ALIGN_CENTER)
            self.question_label.Wrap(650)
            vbox.Add(self.question_label, 0, wx.ALL | wx.EXPAND, 20)

            self.answer_entry = wx.TextCtrl(panel, style=wx.TE_PROCESS_ENTER)
            self.answer_entry.Bind(wx.EVT_TEXT_ENTER, self.on_submit)
            vbox.Add(self.answer_entry, 0, wx.ALL | wx.ALIGN_CENTER, 10)

            self.feedback_label = wx.StaticText(panel, label="")
            vbox.Add(self.feedback_label, 0, wx.ALL | wx.ALIGN_CENTER, 10)
            ...
            self.hint_btn = wx.Button(panel, label="Hint")
            self.hint_btn.Bind(wx.EVT_BUTTON, self.on_hint)
            btn_sizer.Add(self.hint_btn, 0, wx.ALL, 5)

            self.skip_btn = wx.Button(panel, label=f"Skip ({self.skips})")
            self.skip_btn.Bind(wx.EVT_BUTTON, self.on_skip)
            btn_sizer.Add(self.skip_btn, 0, wx.ALL, 5)

            vbox.Add(btn_sizer, 0, wx.ALIGN_CENTER, 10)

            self.score_label = wx.StaticText(panel, label="Score: 0")
            vbox.Add(self.score_label, 0, wx.ALL | wx.ALIGN_CENTER, 10)
        ...
        def load_riddles(self, filename="riddles.csv"):
            ...
            try:
                ...
                wx.MessageBox(f"Could not find '{filename}'\nLooking in: {script_dir}", "Error", wx.OK | wx.ICON_ERROR)
                return []
        ...
        def load_question(self):
            ...
            self.question_label.SetLabel(f"Q{self.index + 1}: {question}")
            self.question_label.Wrap(650)
            self.answer_entry.SetValue("")
            self.feedback_label.SetLabel("")
            self.answer_entry.SetFocus()
            self.start_time = time.time()

        def on_submit(self, event):
            user_answer = self.answer_entry.GetValue().lower().strip()
            if not user_answer:
                return
            ...
            if user_answer == correct_answer:
                ...
                if self.streak % 3 == 0:
                    points += 5
                self.feedback_label.SetLabel(f"Correct! +{points} points")
                self.feedback_label.SetForegroundColour(wx.Colour(0, 128, 0))
                self.score += points
            else:
                self.feedback_label.SetLabel(f"Wrong! Correct answer: {correct_answer}")
                self.feedback_label.SetForegroundColour(wx.Colour(255, 0, 0))
                self.streak = 0
            ...
            if self.index % 5 == 0 and self.index < len(self.riddles):
                wx.MessageBox(f"After {self.index} questions, your score is: {self.score}", "Progress", wx.OK | wx.ICON_INFORMATION)

            wx.CallLater(1200, self.load_question)

        def on_hint(self, event):
            hint = self.riddles[self.index]["hint"]
            if hint:
                wx.MessageBox(hint, "Hint", wx.OK | wx.ICON_INFORMATION)
                self.score -= 3
                self.score_label.SetLabel(f"Score: {self.score}")
            else:
                wx.MessageBox("No hint available for this question.", "Hint", wx.OK | wx.ICON_INFORMATION)

        def on_skip(self, event):
            if self.skips > 0:
                self.skips -= 1
                self.skip_btn.SetLabel(f"Skip ({self.skips})")
                self.index += 1
                self.load_question()
            else:
                wx.MessageBox("You have no skips left!", "No Skips", wx.OK | wx.ICON_WARNING)

        def end_quiz(self):
            wx.MessageBox(f"Your final score: {self.score}", "Quiz Finished", wx.OK | wx.ICON_INFORMATION)
            self.Destroy()
